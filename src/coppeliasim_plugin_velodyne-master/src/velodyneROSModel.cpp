...
CVelodyneROSModel::CVelodyneROSModel(const int visionSensorHandles[4],float frequency,int options,float pointSize,float coloringDistances[2],float scalingFactor,int newPointCloudHandle, int local_frame_handle, int frame_name)
{
    for (int i=0;i<4;i++)
    ...
    temp_node = ROS_server::getNode();
    // _pubVelodyne =ROS_server::getPublisher();
    _pubVelodyne = temp_node.advertise<sensor_msgs::PointCloud2>("/robot_"+std::to_string(frame_name)+"/velodyne/points2", 1);
    ...
    _buffer.header.frame_id="vlp16_sensor_" +std::to_string(frame_name);
