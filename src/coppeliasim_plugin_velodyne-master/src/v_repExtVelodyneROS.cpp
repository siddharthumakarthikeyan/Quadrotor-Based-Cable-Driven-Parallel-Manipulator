...
void LUA_CREATEVELODYNEROSMODEL_CALLBACK(SScriptCallBack* p)
{
    ...
    if (D.readDataFromStack(p->stackID,inArgs_CREATEVELODYNEROSMODEL,inArgs_CREATEVELODYNEROSMODEL[0]-6,LUA_CREATEVELODYNEROSMODEL_COMMAND)) // -5 because the last 5 arguments are optional
    {
        ...
        if (inData->size()>7)
        {
            frame_name=inData->at(7).int32Data[0];
        }

        CVelodyneROSModel* obj=new CVelodyneROSModel(visionSensorHandes,frequency,options,pointSize,coloringDistances,scalingFactor,pointCloudHandle, LocalFrameHandle, frame_name);
